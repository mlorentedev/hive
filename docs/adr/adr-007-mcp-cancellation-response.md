...
**Amended 2026-09-24 (#3, [#434](https://github.com/mlorentedev/hive/issues/434))** — hive now requires `mcp>=2.2,<3`, and the `RequestResponder.respond` patch is deleted. `RequestResponder` no longer exists on 2.x; its dispatcher (`mcp/shared/jsonrpc_dispatcher.py`, `PeerCancelMode`) never answers a cancelled request, so neither the duplicate response nor the respond-after-cancel crash can occur. The python-sdk#2416 repro crashes 3/3 on 1.x without the patch and 0/3 on 2.2.0; `tests/test_cancel_race.py` gates on it, replacing the diagnostic classifier cited in Amendment #2. What changes for this ADR: on 2.x the client that cancelled gets **no** `ErrorData` ack, and the `cancellation` source of `ghost_responses` is no longer recorded; the `deadline` source (ADR-008) is. The semantic rule stands — a cancellation does not imply rollback; verify via `vault_query`, do not retry. Whether a 2.x signal (such as the dispatcher's unanswered-request hook) should feed the `cancellation` source, and where the counter lives once `_compat.py` holds nothing else, is left to [#442](https://github.com/mlorentedev/hive/issues/442).
