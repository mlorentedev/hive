...
- **Still open.** `_compat.py` keeps only `GHOST_RESPONSES`, which `vault_health` reports as `ghost_responses` (ADR-007). Its `deadline` source still fires; nothing records its `cancellation` source on 2.x. Where the counter lives and what feeds that source is [#442](https://github.com/mlorentedev/hive/issues/442).
