...
- **Recommended**
  ...
  - The [obsidian-git plugin](https://github.com/Vinzent03/obsidian-git) with auto-commit set to **5–10 minutes**. The git workload is already off the synchronous tool path — writes defer to a reconciler tick by default — but the plugin still earns its place; see *Recommended configuration* below.
...
3. Nothing to do per call. Writes defer to the reconciler tick by default, so bulk operations already pay for one commit instead of N. Step 3 used to be "pass `commit=False` everywhere" — that flag is now the default and the keyword is redundant.
4. Optionally call `vault_commit(message="...")` at the end of a session to force a checkpoint sooner than either tick.
5. Optionally set `HIVE_AUTO_DEFER_TO_EXTERNAL_COMMITTER=true` to have the reconciler hand off to the plugin instead of committing, whenever the plugin is present and healthy.

`vault_health` reports a `## external_committer` block when it detects obsidian-git in the vault. The durability contract is explicit and unchanged in substance: files are persisted to disk regardless; only the *commit* is deferred. A crash before the next flush loses the commit, not the content — and nothing recovers it automatically, so the `uncommitted` count in the runtime block is where you go looking. See [Deferred commits](/tools/vault/#deferred-commits-ack-semantics).
...
| `HIVE_UPGRADE_POLL_S` | `30.0` | Daemon mode only (`hive serve`): seconds between version-drift checks. On drift the daemon exits `75` so the supervisor restarts it into the new version. Lower = adopts a new version sooner; validated `>0..3600`. See the [daemon mode guide](/hive/guides/daemon-mode/). |
...
| `HIVE_LOG_RETENTION_DAYS` | `7` | How long a per-process debug log is kept. Each process writes its own `hive-<pid>.log` (size-capped at 1MB), so this bounds the *number* of files rather than their size. On startup, a log is deleted only when it is older than this **and** its owning process is gone — an idle daemon's log is never collected. Validated 1..365. |
