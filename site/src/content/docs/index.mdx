...
Your assistant writes to the vault → Obsidian Git auto-commits → next session, everything is there. No manual sync. Write-heavy flows need no extra flags: Hive defers commits to a reconciler tick by default, and can hand off to the plugin entirely; see [Configuration → Recommended configuration](/configuration/#recommended-configuration).
