...
- [x] Implement transparent fallback path **[done — slice 2]** + client auto-reconnect when daemon returns **[done — slice 3, closes M1]**: the proxy resolves its backend through a reconnecting `client_factory` (`_ReconnectingBackend` in `src/hive/_client.py`) that re-reads the published port+token on EVERY forwarded call (`FastMCPProxy._get_client`), so a daemon that dies and restarts on a new port+token is followed without restarting the shim, and a daemon that dies without returning degrades to the cached in-process owner. Prefer-daemon routing keeps the single-owner invariant (only the daemon gets calls while reachable; the in-process owner is built lazily, only on first unreachability). Two TDD tests: `test_client_reconnects_to_restarted_daemon_without_duplicate_write` (A dies mid-session → B restarts on a new port+token → shim follows; a retried `idempotency_key` write is deduped across the restart — proving reconnect is write-safe) + `test_client_degrades_in_process_when_daemon_dies_mid_session` (daemon gone → in-process degrade). Composes with slice 1.2 (killed A frees the flock → B reacquires + self-heals) and slice 2 (shared `idempotency.db` makes the retry a no-op). (2026-06-02, `feat/client-auto-reconnect`)
...
> - **M1 [fixed — slice 3].** The mode decision is no longer one-shot: the proxy's per-call `client_factory` re-reads the published state on every forwarded request, so once the proxy is engaged the shim follows a restarted daemon (new port+token) and degrades to the in-process owner if the daemon dies, recovering when it returns. A *cold* start with no daemon still serves in-process directly (today's behavior, unchanged) — the one remaining one-shot is benign because the supervised daemon is up before sessions spawn. Closed by `_ReconnectingBackend` in `src/hive/_client.py`.
...
- [x] **DR:** restart-on-upgrade — owned `uvicorn.Server` (built from the public `mcp.http_app()`, replacing `mcp.run(transport="http")`) + a `settings.upgrade_poll_s` drift poll (`importlib.metadata.version("hive-vault")` vs the boot snapshot) → `should_exit` cooperative stop → **exit `75` (EX_TEMPFAIL)** so the `Restart=on-failure` unit relaunches into the new code (a signal stop / singleton-decline exits 0 instead — no `RestartPreventExitStatus=` coupling). **Clean-stop only:** a true in-flight DRAIN is unreachable (streamable-http cancels the active handler — spike `spike/upgrade_spike.py`, 2/2 PASS), so the cut call's safety rests on idempotency (slice 2) + auto-reconnect (slice 3). New `_daemon.py` fns `_current_version`/`_upgrade_detected`/`_watch_for_upgrade`/`_serve_until_drift_or_signal`/`_serve_owned`; consts `EXIT_RESTART_ON_UPGRADE=75` + `GRACEFUL_SHUTDOWN_S=2`; new env `HIVE_UPGRADE_POLL_S` (default 30s); `main()` re-raises `SystemExit` before the CRITICAL crash log. 8 unit tests in `tests/test_daemon.py` (predicate, watcher drift/external-stop, exit-code map); rollback counts as drift, the `<not-found>` swap-window sentinel never does. (2026-06-02, `feat/restart-on-upgrade`)
...
- [x] Service install/lifecycle: `hive service {install,uninstall,status}` CLI (new `src/hive/_service.py`). **systemd `--user` unit** (`Restart=on-failure`, `WantedBy=default.target`, bakes `VAULT_PATH` + optional `EnvironmentFile=-`) AND **Windows Scheduled Task** (`LogonTrigger` + `RestartOnFailure`) BOTH real — hive ships on PyPI independent of dotfiles, so it must work cross-OS (ubuntu+windows CI matrix validates the real `systemctl`/`schtasks` calls); **macOS/launchd = clear non-zero stub** that prints the manual step (follow-up, not in the matrix). Both supervisors consume the slice 1.3 exit-75 contract (restart on non-zero, not on clean exit 0). SSOT = the package CLI (dotfiles activation just calls `hive service install`). 9 unit tests (pure renderers + mocked install/uninstall/status + CLI dispatch). User-facing site docs for the daemon CLI deferred to the activation slice (cohesive "daemon mode" page). (2026-06-02, `feat/service-units`)
