...
- [ ] [AC1] Route `vault_write` / `vault_patch` through the queue; assert by test that no `git commit` occurs in the tool's call path. **Decide here where the reconciler thread starts.** `CommitReconciler.__init__` currently starts it eagerly, which is fine standalone but means every process constructing a `ServerContext` gets a live reconciler — including short-lived CLI paths (`hive service install`, `--version`) that build a context and never serve. Check whether those construct a context at all, and if so defer the thread to a `start()` called at serve time
