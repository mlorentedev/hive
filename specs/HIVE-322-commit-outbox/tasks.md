...
- [ ] [AC6] Implement drain-on-shutdown on **two** triggers, since the reconciler is no longer daemon-only: the daemon's existing signal handling, and the stdio server's lifespan teardown / stdin EOF. A client `SIGKILL` gets no drain — that path is covered by AC11's observability, not by AC6. **Note from AC1:** `ServerContext.close()` now drains the reconciler first, but `ctx.close()` is currently called only from tests — nothing in production invokes it. Both triggers therefore have to be wired for real; the drain is not reachable yet
